"""Patch Read_H-Reflex_App_Simplified.ipynb cell 3bdf3a5b with pure Plotly Option 3."""
import json, pathlib

NB_PATH = pathlib.Path(__file__).parent / "Read_H-Reflex_App_Simplified.ipynb"

# 9 traces per amplitude:  sd_lo, sd_hi, avg_bip, m_peak, h_peak, abs_bip, uni, abs_uni, adc
...
if len(hrs2_trials) > 0:
    ...
    for _amp in _sorted_amps:
        ...
        _mm = (_t_ref >= M_WAVE_START_MS) & (_t_ref <= _M_WAVE_END_MS)
    ...
    def _shapes_for(d):
        return [
            ...
            dict(type='rect', xref='x', yref='paper', layer='below',
                 x0=M_WAVE_START_MS, y0=0, x1=_M_WAVE_END_MS, y1=1,
                 fillcolor='rgba(0,0,200,0.10)', line_width=0),
            dict(type='rect', xref='x', yref='paper', layer='below',
                 x0=_H_WAVE_START_MS, y0=0, x1=_H_WAVE_END_MS, y1=1,
    ...
    for _i, _d in enumerate(_amp_data):
        ...
        _v0  = (_i == 0)   # initial visibility for first amplitude
        ...
        # +1 SD upper band (fills to previous trace = lower)
        fig_wave.add_trace(go.Scatter(
    ...
    for _i, _d in enumerate(_amp_data):
        _vis = [False] * _n_total
        _base = _N_PER * _i
        # Main traces: always visible when this amp is selected
        _vis[_base + 0] = True
        _vis[_base + 1] = True
        _vis[_base + 2] = True
        _vis[_base + 3] = True
        _vis[_base + 4] = True
        # Overlay traces: in legend but not drawn (user clicks to show)
...
if len(hrs2_trials) > 0:
    ...
    # ---- RC Figure 1: raw amplitude vs stim current ----
    fig_rc1 = go.Figure()
...
# Each file is self-contained and interactive (no server or kernel needed).
# The 'cdn' option loads Plotly.js from a CDN — requires internet on first open.
# Use include_plotlyjs=True for fully offline files (larger file size).
...
# To export the full notebook as HTML (run from terminal):
#   jupyter nbconvert --to html --no-input Read_H-Reflex_App_Simplified.ipynb
'''
...
for c in nb['cells']:
    ...
    if cid == '3bdf3a5b':
        ...
        patched += 1
    # Use last empty cell for export
    if cid == 'fa8310ce':
